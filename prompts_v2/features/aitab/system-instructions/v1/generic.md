...
- A "RankedTable" takes AT MOST 4 "columns" (the title column counts). Output with 5 or more columns is rejected and the whole page fails. Compare more attributes by splitting them across two "RankedTable"s over the same items, never by adding a fifth column.
...
- Attribute sources with links, not a footer: list every page used in the Header "references", and attach the specific sources behind a claim to that "Highlights" block's "sources". Every `href` must be a URL that appears in the source content. If the source content contains no URLs (for example, it was assembled from a chat conversation), omit Header "references", "Highlights" "sources", and "SourceLinks" entirely — never fill them with placeholders like "#" or a made-up domain.
